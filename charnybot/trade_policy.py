...
class TradingPolicyMostBasic(TradingPolicy):
    ...
    def score_tickers(self, date, tickers_df, complement_df):
        ...
        tickers_not_in_portfolio = set(complement_df[complement_df.Date.dt.normalize() == date].ticker) - set(
    ...
    def score_ticker(self, date, ticker, tickers_df, complement_df, portfolio_weight):
        ...
        price_above_ma200 = ticker_df.ma_200.values[-1] < ticker_df.Close.values[-1]

        # price condition to buy
        if price_above_ma200 & price_above_ma150 & ma_slop_is_positive:
            # for now  , only pass/fail  score TODO - revisit
    ...
    def buy(self, date, tickers_score, tickers_df, default_index_df):
        ...
        if number_of_new_new_tickers_to_add > 0:
            ...
            if number_of_stocks_in_portfolio:
                ...
                new_ticker_weight = np.min(
                    [average_ticker_weight, self.config.get_parameter('portfolio', 'max_portion_per_ticker')])
        ...
        for ticker in new_portfolio_weights.keys():
            new_portfolio_weights[ticker] = np.min(
                [new_portfolio_weights[ticker], self.config.get_parameter('portfolio', 'max_portion_per_ticker')])
    ...
    def sell(self, date, ticker, tickers_df, complement_df, portfolio_weight):
        ...
        # Sell condition
        tdf = tickers_df[(tickers_df.ticker == ticker)].sort_values('Date')
        price = tdf.Close.values
        ma_200 = tdf.ma_200.values

        current_ind = np.where(tdf.Date == date)[0][0]

        # Condition - 2 days that the price is under ma200
        sell_condition = ~np.isnan(ma_200[current_ind]) & (ma_200[current_ind] > price[current_ind]) & (
                    ma_200[current_ind - 1] > price[current_ind - 1])

        if sell_condition:
            # Sell all holdings for this ticker
            ticker_price = price[current_ind]
    ...
    def trade_recurrent(self, date, tickers, tickers_df, complement_df, default_index):
        ...
        self.portfolio.update_prices(tickers_df[tickers_df.Date.dt.normalize() == date],
                                     default_index[default_index.Date.dt.normalize() == date], date)
        ...
        ######################################################################################################
        self.portfolio.sell_all_default_index(default_index[default_index.Date.dt.normalize() == date].Close.values[0],
                                              date)
        ...
        self.buy(date, tickers_score, tickers_df[tickers_df.Date.dt.normalize() == date],
                 default_index[default_index.Date == date])
...
# @TradingPolicy.register("MoreBasic")
# class TradingPolicyMoreBasic(TradingPolicyMostBasic):
#     def __init__(self ,config, default_index_name = 'snp' ):
#         self.name = "MoreBasic"
#         self.config = config
#         self.portfolio = Portfolio()
...
#             if ticker not in current_portfolio_weights:
#                 # new ticker
#                 value_per_stock = 100/1 # allways same price
#                 ticker_price = tickers_df[tickers_df.ticker == ticker].Close.values[0]
#                 quantity = value_per_stock / ticker_price
#                 self.portfolio.buy_stock(ticker, quantity, ticker_price)
#             # else:
...
#
#         # Condition - 2 days that the price is under ma200
#         sell_condition  = ~np.isnan(ma_200[current_ind]) &  (ma_200[current_ind] > price[current_ind]) & (ma_200[current_ind-1] > price[current_ind-1])
#
#
#         if sell_condition:
#             # Sell all holdings for this ticker
...
#
#         # Update existing stocks in the portfolio
#         self.portfolio.update_prices(tickers_df[tickers_df.Date.dt.normalize() == date] , default_index[default_index.Date.dt.normalize() == date] , date)
#
#         portfolio_weights = self.portfolio.get_portfolio_weights()
#
#         ######################################################################################################
...
#         #  will convert the unused  cash back to the reference index at the end of this day
#         ######################################################################################################
#         self.portfolio.sell_all_default_index(default_index[default_index.Date.dt.normalize() == date].Close.values[0] , date)
#
#         ###################################################
#         # sell tickers that does not meet price criteria
#         ###################################################
